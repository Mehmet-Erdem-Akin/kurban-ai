        ...
        *Bunlar ÖRNEK referans değerlerdir. Fotoğraftaki spesifik hayvanı analiz et ve gerçek durumuna göre bireysel değerlendirme yap.*
        ...
        try {
            ...
            if (jsonMatch) {
                ...
                if (!analysis.animalType || !analysis.breed || !analysis.estimatedWeight) {
        ...
        return {
            ...
            confidence: 88,
            physicalCondition: 'Sağlıklı görünümde, iyi beslenmiş dana. Kas yapısı gelişmiş, tüy parlak.',
            recommendations: [
        ...
        const randomBreed = randomAnimalType === 'Dana'
            ? (Math.random() > 0.5 ? 'Holstein' : 'Simental')
            : (Math.random() > 0.5 ? 'Akkaraman' : 'Merinos');
        const randomWeight = randomAnimalType === 'Dana'
        ...
        const estimatedPrice = Math.floor((randomWeight / 2) * baseMeatPrice * (1 + Math.random() * 0.4)); // +0-40% premiums

        console.log('🎲 Mock data oluşturuldu:', { randomAnimalType, randomBreed, randomWeight, estimatedPrice });
        ...
        return {
            ...
            physicalCondition: randomAnimalType === 'Dana'
            ...
            recommendations: [
                ...
                randomAnimalType === 'Dana' ? 'Büyükbaş için uygun kalite' : 'Küçükbaş için ideal boyut',
                'Kurban bayramı için uygun',
                Math.random() > 0.5 ? 'Premium ırk özellikleri mevcut' : 'Standart kalite seviyesinde'
            ]
        };
    }
};
...
const calculateDetailedAnalysis = (basicAnalysis: any) => {
    ...
    const karkasWeight = Math.floor(estimatedWeight * (karkasYieldPercentage / 100));
    const bonelessWeight = Math.floor(karkasWeight * (bonelessYieldPercentage / 100));
    ...
    const finalMarketPrice = marketPrice && marketPrice > 0 ? marketPrice : calculatedMarketPrice;
    ...
    if (healthScore > 90) recommendations.push("Mükemmel sağlık durumu - premium kalite");
    ...
    if (finalMarketPrice > expectedPrice * 1.2) recommendations.push("Piyasa ortalaması üstü fiyat");
    else if (finalMarketPrice < expectedPrice * 0.8) recommendations.push("Uygun fiyat - iyi değer");
    ...
    try {
        ...
        if (analysisType === 'multiple' && images && Array.isArray(images)) {
            ...
            try {
                ...
                console.log(`✅ Aynı hayvana ait ${images.length} fotoğraf başarıyla analiz edildi`);
                ...
                console.log(`⚠️ Fallback ile ${images.length} fotoğraf analizi tamamlandı`);
