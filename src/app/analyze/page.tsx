    ...
    const handleAnalyze = async () => {
        ...
        try {
            ...
            if (result.success === false || result.error) {
                ...
                switch (result.error) {
                    case "NO_ANIMAL_DETECTED":
                        errorMessage = "Fotoğrafta kurbanlık hayvan tespit edilemedi. Lütfen hayvanın net bir şekilde görünür olduğu bir fotoğraf yükleyin.";
                        break;
                    ...
                    case "INCOMPLETE_ANALYSIS":
                        errorMessage = "Hayvan net olarak tespit edilemedi. Lütfen daha net ve açık bir fotoğraf yükleyin.";
                        break;
                    case "ANALYSIS_FAILED":
                        errorMessage = "Fotoğraf analizi başarısız oldu. Lütfen farklı bir fotoğraf deneyin veya tekrar yükleyin.";
                        break;
                    case "API_ERROR":
                        errorMessage = "Yapay zeka servisinde geçici bir sorun oluştu. Lütfen bir dakika sonra tekrar deneyin.";
                        break;
                    default:
                        errorMessage = result.message || "Analiz sırasında bir hata oluştu.";
                }
            ...
            setAnalysisError({
                ...
                message: "Analiz sırasında beklenmeyen bir hata oluştu. Lütfen internet bağlantınızı kontrol edin ve tekrar deneyin.",
                analysisType: "single",
                totalImages: selectedImages.length,
            });
        } finally {
    ...
    const handleAnalyzeAll = async () => {
        ...
        try {
            ...
            if (result.success === false || result.error) {
                ...
                switch (result.error) {
                    case "NO_ANIMAL_DETECTED":
                        errorMessage = "Fotoğraflarda kurbanlık hayvan tespit edilemedi. Lütfen hayvanın net bir şekilde görünür olduğu fotoğraflar yükleyin.";
                        break;
                    case "INVALID_ANIMAL_TYPE":
                        ...
                        break;
                    case "INCOMPLETE_ANALYSIS":
                        errorMessage = "Hayvan fotoğraflarda net olarak tespit edilemedi. Lütfen daha net ve açık fotoğraflar yükleyin.";
                        break;
                    case "ANALYSIS_ERROR":
                        errorMessage = "Çoklu fotoğraf analizi başarısız oldu. Lütfen farklı fotoğraflar deneyin veya tekrar yükleyin.";
                        break;
                    case "API_ERROR":
                        errorMessage = "Yapay zeka servisinde geçici bir sorun oluştu. Lütfen bir dakika sonra tekrar deneyin.";
                        break;
                    default:
                        errorMessage = result.message || "Çoklu analiz sırasında bir hata oluştu.";
                }
            ...
            setAnalysisError({
                errorType: "NETWORK_ERROR",
                message: "Çoklu fotoğraf analizi sırasında beklenmeyen bir hata oluştu. Lütfen internet bağlantınızı kontrol edin ve tekrar deneyin.",
                analysisType: "multiple",
                totalImages: selectedImages.length,
            });
        } finally {
                        ...
                        <div className="card p-8 animate-scale-in border-red-200 bg-red-50">
                            <div className="text-center">
                                ...
                                <h3 className="text-2xl font-bold text-red-900 mb-4">
                                    {analysisError.errorType === "NO_ANIMAL_DETECTED" ? "🚫 Hayvan Tespit Edilemedi" :
                                        analysisError.errorType === "INVALID_ANIMAL_TYPE" ? "⚠️ Geçersiz Hayvan Türü" :
                                            analysisError.errorType === "INCOMPLETE_ANALYSIS" ? "🔍 Analiz Eksik" :
                                                analysisError.errorType === "ANALYSIS_FAILED" ? "❌ Analiz Başarısız" :
                                                    analysisError.errorType === "API_ERROR" ? "🔧 Servis Hatası" :
                                                        analysisError.errorType === "NETWORK_ERROR" ? "🌐 Bağlantı Hatası" :
                                                            "❌ Analiz Hatası"}
                                </h3>
                                ...
                                {analysisError.detectedType && (
                                    <div className="bg-red-100 border border-red-300 rounded-lg p-4 mb-6 max-w-md mx-auto">
                                        <p className="text-sm text-red-700">
                                            <span className="font-semibold">Tespit edilen:</span> {analysisError.detectedType}
                                        </p>
                                    </div>
                                )}
                                ...
                                <div className="mt-8 p-4 bg-blue-50 border border-blue-200 rounded-lg max-w-2xl mx-auto">
                                    ...
                                    <div className="text-sm text-blue-800 text-left space-y-2">
                                        {analysisError.errorType === "NO_ANIMAL_DETECTED" && (
                                            <>
                                                <p>• Hayvanın tüm vücudunun görünür olduğundan emin olun</p>
                                                <p>• Fotoğrafın net ve aydınlık olduğunu kontrol edin</p>
                                                <p>• Hayvan fotoğrafın merkezinde olmalı</p>
                                            </>
                                        )}
                                        {analysisError.errorType === "INVALID_ANIMAL_TYPE" && (
                                            <>
                                                <p>• Sadece kurbanlık hayvanlar: Dana, Boğa, İnek, Koç, Koyun, Keçi, Manda</p>
                                                <p>• Evcil hayvanlar (kedi, köpek) analiz edilemez</p>
                                                <p>• Vahşi hayvanlar desteklenmez</p>
                                            </>
                                        )}
                                        {(analysisError.errorType === "INCOMPLETE_ANALYSIS" || analysisError.errorType === "ANALYSIS_FAILED") && (
                                            <>
                                        ...
                                        {(analysisError.errorType === "API_ERROR" || analysisError.errorType === "NETWORK_ERROR") && (
                                            <>
                            ...
                            <div className="mb-6">
                                <div className="card p-6">
                                    <div className="text-center mb-6">
                                        ...
                                        <p className="text-neutral-600">
                                            Hayvan hakkında bilgi vermek AI analizinin doğruluğunu %30-40 artırır. Bu adım opsiyoneldir, istediğiniz kadar doldurun.
                                        </p>
                                    </div>
                                    ...
                                    <div className="bg-blue-50 p-6 rounded-lg border border-blue-200">
                                        <div className="grid md:grid-cols-2 gap-4">
                                            ...
                                            <div>
                                                ...
                                                <select
                                                    ...
                                                    onChange={(e) => handleAdditionalInfoChange('animalCategory', e.target.value)}
                                                    className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
                                                >
                                                    <option value="">Seçiniz</option>
                                                    <option value="büyükbaş">🐄 Büyükbaş (Sığır, Dana vb.)</option>
                                                    <option value="küçükbaş">🐑 Küçükbaş (Koyun, Keçi vb.)</option>
                                                </select>
                                            </div>
                                            ...
                                            <div>
                                                ...
                                                <select
                                                    ...
                                                    onChange={(e) => handleAdditionalInfoChange('animalType', e.target.value)}
                                                    className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
                                                >
                                                    <option value="">Seçiniz</option>
                                                    {additionalInfo.animalCategory === 'büyükbaş' ? (
                                                        ...
                                                        <option value="" disabled>Önce kategori seçiniz</option>
                                                    )}
                                                </select>
                                            </div>
                                            ...
                                            <div>
                                                ...
                                                <select
                                                    ...
                                                    onChange={(e) => handleAdditionalInfoChange('gender', e.target.value)}
                                                    className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
                                                >
                                                    <option value="">Seçiniz</option>
                                            ...
                                            <div>
                                                ...
                                                <select
                                                    ...
                                                    onChange={(e) => handleAdditionalInfoChange('estimatedAge', e.target.value)}
                                                    className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
                                                >
                                                    <option value="">Seçiniz</option>
                                            ...
                                            <div>
                                                ...
                                                <input
                                                    ...
                                                    onChange={(e) => handleAdditionalInfoChange('weight', e.target.value)}
                                                    className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
                                                />
                                            </div>
                                            ...
                                            <div>
                                                ...
                                                <select
                                                    ...
                                                    onChange={(e) => handleAdditionalInfoChange('healthCondition', e.target.value)}
                                                    className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
                                                >
                                                    <option value="">Seçiniz</option>
                                            ...
                                            {additionalInfo.gender === 'dişi' && (
                                                <div>
                                                    ...
                                                    <select
                                                        ...
                                                        onChange={(e) => handleAdditionalInfoChange('pregnancyStatus', e.target.value)}
                                                        className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
                                                    >
                                                        <option value="">Seçiniz</option>
                                            ...
                                            <div>
                                                ...
                                                <input
                                                    ...
                                                    onChange={(e) => handleAdditionalInfoChange('region', e.target.value)}
                                                    className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
                                                />
                                            </div>
                                        </div>
                                        ...
                                        <div className="mt-4 p-3 bg-green-50 border border-green-200 rounded-lg">
                                            <p className="text-xs text-green-700 text-center">
                                                💡 <strong>İpucu:</strong> Ne kadar çok bilgi verirseniz, AI analizi o kadar doğru olur!
                                            </p>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div className="card">
                                    ...
                                    <div className="card-body space-y-4">
                                        ...
                                        {(() => {
                                            ...
                                            const buyukbasHayvanlar = ["Dana", "Tosun", "Boğa", "İnek", "Manda", "Buzağı", "Sığır"];
                                            // Küçükbaş hayvanlar listesi  
                                            const kucukbasHayvanlar = ["Koyun", "Keçi", "Oğlak", "Kuzu", "Teke"];

                                            const isBuyukbas = buyukbasHayvanlar.some(hayvan =>
                                                analysisResult?.animalType?.toLowerCase().includes(hayvan.toLowerCase())
                                            );
                                            ...
                                            if (isBuyukbas) {
                                                ...
                                                return (
                                                    <>
                                                        <div className="flex justify-between items-center">
                                                            <span className="text-neutral-600">Hisse Sayısı:</span>
                                                            <span className="font-semibold">7 kişilik</span>
                                                        </div>
                                                        <div className="flex justify-between items-center">
                                                            <span className="text-neutral-600">Hisse Başı:</span>
                                                ...
                                                return (
                                                    <div className="flex justify-between items-center">
                                                        <span className="text-neutral-600">Hisse Durumu:</span>
                                                        <span className="font-semibold text-amber-700">Tek hisse (bölünemez)</span>
                                                    </div>
                                                );
                                            } else {
                                                // 🐾 Diğer hayvanlar
                                                return (
                                                    <div className="flex justify-between items-center">
                                                        <span className="text-neutral-600">Hisse Durumu:</span>
                                                        <span className="font-semibold text-gray-700">Değişken</span>
                                                    </div>
                                                );
                                            }
                                        })()}
                                        ...
                                        <div className="bg-gray-50 p-3 rounded-lg">
                                            <div className="flex justify-between items-center mb-2">
                                                <span className="text-neutral-600">Canlı Ağırlık Fiyatı:</span>
                                                <span className="font-bold text-gray-700">
                            ...
                            <div className="grid md:grid-cols-2 gap-6">
                                ...
                                <div className="card">
                                    ...
                                    <div className="card-body space-y-3">
                                        ...
                                        {Object.keys(additionalInfo).some(key => additionalInfo[key as keyof AdditionalInfo]) && (
                                            <>
                                                <hr className="my-3" />
                                                <div className="text-sm">
                                                    ...
                                                    <div className="bg-green-50 p-3 rounded-lg space-y-1">
                                                        {additionalInfo.animalCategory && (
                                                            <div className="flex justify-between text-xs">
                                                                ...
                                                                <span className="font-medium">
                                                                    {additionalInfo.animalCategory === 'büyükbaş' ? '🐄 Büyükbaş' : '🐑 Küçükbaş'}
                                                                </span>
                                                            </div>
                                                        )}
                                                        {additionalInfo.animalType && (
                                                            <div className="flex justify-between text-xs">
                                                                <span>Tür:</span>
                                                                <span className="font-medium">{additionalInfo.animalType}</span>
                                                            </div>
                                                        )}
                                                        {additionalInfo.gender && (
                                                            <div className="flex justify-between text-xs">
                                                                <span>Cinsiyet:</span>
                                                                <span className="font-medium">
                                                                    {additionalInfo.gender === 'erkek' ? '♂️ Erkek' : '♀️ Dişi'}
                                                                </span>
                                                            </div>
                                                        )}
                                                        {additionalInfo.estimatedAge && (
                                                            <div className="flex justify-between text-xs">
                                                                <span>Yaş:</span>
                                                                <span className="font-medium">{additionalInfo.estimatedAge}</span>
                                                            </div>
                                                        )}
                                                        {additionalInfo.healthCondition && (
                                                            <div className="flex justify-between text-xs">
                                                                <span>Sağlık:</span>
                                                                <span className="font-medium">{additionalInfo.healthCondition}</span>
                                                            </div>
                                                        )}
                                                        {additionalInfo.pregnancyStatus && (
                                                            <div className="flex justify-between text-xs">
                                                                <span>Gebelik:</span>
                                                                <span className="font-medium">{additionalInfo.pregnancyStatus}</span>
                                                            </div>
                                                        )}
                                                        {additionalInfo.weight && (
                                                            <div className="flex justify-between text-xs">
                                                                <span>Tahmini Ağırlık:</span>
                                                                <span className="font-medium">{additionalInfo.weight}</span>
                                                            </div>
                                                        )}
                                                        {additionalInfo.region && (
                                                            <div className="flex justify-between text-xs">
                                                                <span>Bölge:</span>
                                                                <span className="font-medium">{additionalInfo.region}</span>
                                                            </div>
                                                        )}
                                                        {additionalInfo.specialNotes && (
                                                            <div className="text-xs mt-2">
                                                                <span className="font-medium">Özel Notlar:</span>
                                                    ...
                                                    <p className="text-xs text-green-600 mt-2 text-center">
                                                        ✅ Bu bilgiler AI analizinin doğruluğunu artırmak için kullanıldı
                                                    </p>
                                                </div>
                                            </>
                                        )}
